  ...
  if (sections.length === 0) {
    try {
      ...
      const manifestCandidates = [
        path.resolve(rootDir, 'templates', templateVer.templateKey, 'manifest.json'),
        path.resolve(rootDir, 'backend', 'templates', templateVer.templateKey, 'manifest.json'),
        path.resolve(process.cwd(), 'templates', templateVer.templateKey, 'manifest.json')
      ];
