...
publicRoutes.get('/render', async (c) => {
  ...
  try {
    ...
    const possiblePaths = [
      ...
      path.resolve(rootDir, 'backend', 'templates', templateKey, 'index.html'),
      path.resolve(rootDir, 'storage', 'templates', templateKey, '1.0.0', 'index.html'),
      path.resolve(rootDir, 'backend', 'storage', 'templates', templateKey, '1.0.0', 'index.html'),
    ...
    if (resolvedPath) {
      ...
      const candidateDirs = [
        ...
        path.resolve(rootDir, 'backend', 'templates'),
        path.resolve(process.cwd(), 'templates')
      ];
      for (const templatesDir of candidateDirs) {
